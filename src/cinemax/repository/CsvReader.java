...
public enum CsvReader {
	...
	/**
	 * Processes the file from the set path with the set processor. Uses UTF-8 as
	 * standard charset.
	 * 
	 ...
	 */
	public boolean process(String path) throws ParsingException {
		...
		try (Stream<String> lines = Files.lines(Paths.get(path), StandardCharsets.UTF_8)) {
			...
			switch (e) {
				...
				case CharacterCodingException _ ->
					throw new ParsingException(ParsingError.CSVPARSER_FILE_CHARCODING_INVALID);
				case CharConversionException _ ->
					throw new ParsingException(ParsingError.CSVPARSER_FILE_CHARCONVERSION_INVALID);
